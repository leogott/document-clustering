...
def order_clusters(matrix):
    ...
    # Order clusters
    _, first = max(
        [(centrality, idx) for idx, centrality in enumerate(sum(simmx))]
    )  # Most central cluster
    queue = [first]
    while len(queue) < N_clusters:
        ...
        sim, next = max(
            [
                (centrality, idx)
                for idx, centrality in enumerate(sum(simmx))
                if idx not in queue
            ]
        )
...
# print("Filtering...")
data_filtered = [
    " ".join([term for term in tp.clean(doc.split(), stem=False)]) for doc in data
]
...
for i, ch in enumerate(order_clusters(km.cluster_centers_)):
    ...
    for j, section in enumerate(order_clusters(km2.cluster_centers_)):
        ...
        docs_in_section = [
            doc for doc, sect in zip(ch_members, km2.labels_) if sect == section
        ]
        top_docs = heapq.nlargest(
            N_docs,
            zip(docs_in_section, sects2docs_simmx[section][docs_in_section]),
            key=lambda x: x[1],
        )
...
for i, chapter in enumerate(chapter_outline):
    ...
    print(
        '  <button class="collapsible"><h1>%d. %s</h1></button>'
        % (i + 1, ", ".join([w for w, _ in top_kws]))
    )
    print('  <div class="content">  ')
    print(
        '    <button class="collapsible"><h2>'
        + str(i + 1)
        + '.1. Introduction</h2></button>   <div class="content"><p>DUMMY_INTRODUCTION_'
        + str(i + 1)
        + "_CONTENT</p></div>"
    )
    for j, section in enumerate(chapter):
        ...
        print(
            '    <button class="collapsible"><h2>%d.%d. %s</h2></button>'
            % (i + 1, j + 2, ", ".join([w for w, _ in top_kws]))
        )
        print('    <div class="content">  ')
        for k, doc in enumerate(section):
            doc_idx, score = doc
            print(
                '      <button class="collapsible"><h3>%d.%d.%d. %s (doc:%d/rel:%.1f%%)</h3></button>   <div class="content"><p>DUMMY_PAPER_%d.%d.%d._CONTENT</p></div>'
                % (
                    i + 1,
                    j + 2,
                    k + 1,
                    data[doc_idx].split("|")[0],
                    doc_idx,
                    score * 100,
                    i + 1,
                    j + 2,
    ...
    print(
        ...
        + "."
        + str(N_sections + 2)
        + '. Conclusion</h2></button>   <div class="content"><p>DUMMY_CONCLUSION_'
        + str(i + 1)
        + "_CONTENT</p></div>"
    )
    print(
        ...
        + "."
        + str(N_sections + 3)
        + '. Related Work</h2></button>   <div class="content"><p>DUMMY_RELATEDWORK_'
        + str(i + 1)
        + "_CONTENT</p></div>"
    )
    print(
        ...
        + "."
        + str(N_sections + 4)
        + '. References</h2></button>   <div class="content"><p>DUMMY_REFERENCES_'
        + str(i + 1)
        + "_CONTENT</p></div>"
    )
    print("  </div>  ")
...
selected_docs = [
    x
    for x, _ in functools.reduce(
        lambda a, b: a + b, functools.reduce(lambda a, b: a + b, chapter_outline)
    )
]
if project_selected_docs_only:
    km2_dists = 1 - cosine_similarity(
        np.concatenate(
            [
                np.array(km2_centroids),
                np.array(km_centroids),
                tfidf_matrix.toarray()[selected_docs],
            ]
        )
    )
else:
    km2_dists = 1 - cosine_similarity(
        np.concatenate(
            [np.array(km2_centroids), np.array(km_centroids), tfidf_matrix.toarray()]
        )
    )
...
plt.scatter(
    pos[: N_chapters * N_sections, 0],
    pos[: N_chapters * N_sections, 1],
    s=200,
    c=functools.reduce(
        lambda a, b: a + b, [[i] * N_sections for i in range(N_chapters)]
    ),
)
plt.scatter(
    pos[N_chapters * N_sections : (N_chapters * (N_sections + 1)), 0],
    pos[N_chapters * N_sections : (N_chapters * (N_sections + 1)), 1],
    s=600,
    c=range(N_chapters),
    alpha=0.3,
)
if project_selected_docs_only:
    plt.scatter(
        pos[(N_chapters * (N_sections + 1)) :, 0],
        pos[(N_chapters * (N_sections + 1)) :, 1],
        s=20,
        c=np.array(order_clusters(km.cluster_centers_))[km.labels_[selected_docs]],
    )
else:
    plt.scatter(
        pos[(N_chapters * (N_sections + 1)) :, 0][selected_docs],
        pos[(N_chapters * (N_sections + 1)) :, 1][selected_docs],
        s=20,
        c=np.array(order_clusters(km.cluster_centers_))[km.labels_[selected_docs]],
    )
...
plt.savefig(os.path.join(OUTPUT_DIR, "corpus.png"))
